...
param_decl        : var_type id {if(!Micro.symbolTables.defineSymbol($id.text,$var_type.text,null,true)) throw new RuntimeException("DECLARATION ERROR "+$id.text);;};
param_decl_tail   : ',' param_decl param_decl_tail | ;

/** Function Declarations */
func_declarations : func_decl func_declarations | ;
func_decl         : FUNCTION any_type id {if(!Micro.symbolTables.defineSymbol($id.text,"FUNCTION",null)) throw new RuntimeException("DECLARATION ERROR "+$id.text); Micro.symbolTables.createNewScope(ScopeType.Fun,$id.text);  Micro.program.createFun(Micro.symbolTables.currTable);}'('param_decl_list')' BEGIN func_body {Micro.symbolTables.popTable();} END;
...
/** Basic Statements */
id_list2  returns[List<id> ids]   locals [List<id> _ids=new ArrayList()]:{$ids=new ArrayList<id>();} id {id tmp=new id(); tmp.type=NodeType.id; Symbol symbol= Micro.symbolTables.Lookup($id.text); if(symbol==null) throw new RuntimeException("Varaiable not  decleared  "+$id.text); tmp.setSymbol(symbol); $ids.add(tmp);}  id_tail2 { for(int i=0;i<$_ids.size();i++) $ids.add($_ids.get(i));  };
id_tail2           : ',' id { id tmp=new id(); tmp.type=NodeType.id; Symbol symbol= Micro.symbolTables.Lookup($id.text); if(symbol==null) throw new RuntimeException("Varaiable not  decleared  "+$id.text); tmp.setSymbol(symbol);    $id_list2::_ids.add(tmp);} id_tail2 | ;
assign_stmt    returns[AstNode node]   : t1=assign_expr';' {$node =$t1.node; };
assign_expr  returns [AstNode node]     : { exp assign_op=new exp();   assign_op.type=NodeType.exp; assign_op.opType=OpType.assign; } t1=id ':=' t2=expr { id id_var =new id(); id_var.type=NodeType.id; Symbol symbol= Micro.symbolTables.Lookup($id.text);     if(symbol==null) throw new RuntimeException("Varaiable not  decleared  "+$id.text); ((id)id_var).setSymbol(symbol);  assign_op.left=id_var; assign_op.right=$t2.node; $node=assign_op;  };
...
if(symbol==null)throw new RuntimeException("Function not  decleared  "+$id.text); 
if(symbol.getType()!=Type.FUN) throw new RuntimeException($id.text + "is not a function !!");  
var.funSymbol=symbol;   
FunctionUnit tmp= Micro.program.getFunFromName($id.text);  
if($t1.node!=null){
if(!(tmp.checkFunParNum($t1.node.size()))) throw new RuntimeException($id.text+ ": check number of parameters");   
}else {
if(!(tmp.checkFunParNum(0))) throw new RuntimeException($id.text+ ": check number of parameters"); 
}
